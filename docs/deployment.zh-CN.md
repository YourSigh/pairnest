...
| `PAIRNEST_ALLOW_OPEN_COUPLE_CREATE` | `false` | 是否允许匿名创建情侣空间；仅在可信私有实例上设为 `true` |
| `PAIRNEST_ALLOW_LEGACY_SHARED_SECRET_ACTIVATE` | `false` | 是否允许旧版 shared secret 激活 `legacy-default-couple`；仅迁移期间临时设为 `true` |
...
等待七天后再次确认。删除会立即移除数据库记录；已登记媒体进入异步清理队列
（响应可能带有 `mediaCleanupPending`，多次失败后会进入死信日志）。同时断开该空间的
活动 WebSocket。独立备份与第三方提供商数据需另行处理。详见 [隐私说明](privacy.zh-CN.md)。

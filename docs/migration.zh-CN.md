...
- 增加绑定相反身份的 24 小时邀请、女方与男方各自独立的恢复密钥、持久限流桶、情侣空间存储额度和数据删除；
...
Compose 的 `migrate` 服务会通过 `prisma migrate deploy` 执行仓库中已提交的三次
...
早期多情侣开发版本曾生成不区分成员的情侣空间级恢复密钥，也可能留下身份不明确或可被
用于接管的邀请。第三次 migration 会主动使这些凭证，以及已配对空间或已历史绑定身份的
旧邀请失效。升级后，两名已登录成员应分别在设置中生成自己的恢复密钥；只有相反身份从
未加入过时才能重新签发邀请，否则该成员必须使用自己的恢复密钥。这个变更不会删除聊天
或其他业务数据。

新版创建接口会在同一请求中绑定创建者并签发会话，`/v1/ping` 通过
`pairingProtocolVersion: 2` 标识该协议。客户端与服务端应一起升级；新版客户端会对未
声明此版本的旧服务端关闭空间创建入口，避免旧接口先创建无主空间后才返回不兼容响应。
客户端生成的 `requestId` 和 Device Session 上的单向回放标记使创建与首次加入可以在
响应丢失后安全重试；这些标记不允许其他设备复用邀请码。
...
它受 IP/设备失败次数限制保护，服务端会校验已有 scrypt 哈希。共享密钥只能首次绑定一个
从未绑定过的成员位置，或让服务端已经确认的同一活跃设备重试；一旦某个身份存在历史
Session 或成员恢复记录，新设备就不能用共享密钥接管，必须使用该成员自己的恢复密钥。
迁移完成后请保持该开关为 `false`，即使 `AuthConfig` 仍在。

公开 PairNest App 不会内置、公开或要求这个旧密钥。只应通过可信的一次性迁移客户端或
运营者控制的请求使用它，然后由两名成员分别建立自己的恢复密钥。不要把旧密钥写入

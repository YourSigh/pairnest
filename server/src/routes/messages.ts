...
messagesRouter.post('/gacha', async (req, res) => {
  ...
  if (!isChatRole(sender)) {
    res.status(400).json({ ok: false, message: 'sender 必须为 female 或 male' });
    return;
  }
...
messagesRouter.post('/voice', voiceUpload.single('audio'), async (req, res) => {
  const file = req.file;
  const sender = getAuthenticatedRole(res);
...
messagesRouter.post('/image', imageUpload.single('image'), async (req, res) => {
  const file = req.file;
  let processedImage: Awaited<ReturnType<typeof processChatImageUpload>> | null = null;
  const sender = getAuthenticatedRole(res);
  ...
  if (!file || !isChatRole(sender) || !Number.isFinite(width) || !Number.isFinite(height)) {
    await removeUploadedFile();
    res.status(400).json({
      ok: false,
...
messagesRouter.post('/:id/transcribe', async (req, res) => {
...
messagesRouter.get('/unread-count', async (req, res) => {
  ...
  const state = await prisma.chatReadState.findUnique({ where: { role } });
...
messagesRouter.post('/read', async (req, res) => {
  ...
  const state = await prisma.$transaction(
    async (tx) => {
      const existing = await tx.chatReadState.findUnique({ where: { role } });

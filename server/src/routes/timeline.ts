...
timelineRouter.get('/', async (req, res) => {
  ...
  if ((from && !isValidTimelineDate(from)) || (to && !isValidTimelineDate(to))) {
  ...
  if (createdBy && !normalizeTimelineRole(createdBy)) {
    res.status(400).json({ ok: false, message: 'createdBy 必须为 female 或 male' });
    return;
  }
...
timelineRouter.post('/', async (req, res) => {
  ...
  const description = normalizeTimelineText(req.body?.description, MAX_DESCRIPTION_LENGTH);
  ...
  const location = normalizeOptionalTimelineText(req.body?.location, MAX_LOCATION_LENGTH);
  const createdBy = getAuthenticatedRole(res);
  const mood = isTimelineMood(req.body?.mood) ? (req.body.mood as TimelineMood) : 'sweet';
...
timelineRouter.post('/:id/image', timelineImageUpload.single('image'), async (req, res) => {
  ...
  try {
    const existing = await prisma.timelineNode.findUnique({ where: { id: nodeId } });
    if (!existing) {
      await removeUploadedFile();
    ...
    if (existing.imageFileName && existing.imageFileName !== file.filename) {
      await unlink(getTimelineImageFilePath(existing.imageFileName)).catch(() => undefined);
    }
    res.status(201).json({ ok: true, item: toTimelineNodeDto(item) });
  } catch (error) {
    await removeUploadedFile();
...
timelineRouter.get('/:id/image', async (req, res) => {
  const item = await prisma.timelineNode.findUnique({ where: { id: req.params.id } });
  ...
  const downloadName = getTimelineImageDownloadName(item.id, item.imageFileName);
...
timelineRouter.delete('/:id/image', async (req, res) => {
  const existing = await prisma.timelineNode.findUnique({ where: { id: req.params.id } });
  if (!existing) {
    res.status(404).json({ ok: false, message: '时间线节点不存在' });
    return;
  }
...
timelineRouter.patch('/:id', async (req, res) => {
  ...
  if (hasOwn(req.body, 'description')) {
    data.description = normalizeTimelineText(req.body.description, MAX_DESCRIPTION_LENGTH);
  }
  ...
  if (hasOwn(req.body, 'location')) {
    data.location = normalizeOptionalTimelineText(req.body.location, MAX_LOCATION_LENGTH);
  }
